# prose-writer lessons

This is durable, harness-neutral working knowledge for the `prose-writer` profile.
...
```bash
python3 lessons.py add --key prose-writer --task "<task type>" \
  --lesson "<reusable lesson>" --evidence "<path, command, or measurement>"
```

and fold them in later with `python3 lessons.py promote --key prose-writer --apply`,
