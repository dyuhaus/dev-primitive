...
<!-- Historical proposal for the retired automatic-merge system. Current machine/project instructions require David to review and merge; this entry grants no automation authority. -->
- 2026-07-31 | autonomy-gate redesign | When the human approver approves everything unread, every "escalate to human" path is functionally "merge after a delay" — so design gates around default-proceed veto windows, bounded machine fix loops, parking with expiry, and cheap reversibility (squash+auto-revert+breaker) instead of decision requests; reserve human interrupts for actions only a human can take (rotate a leaked secret, consent to policy), never for correctness judgments | /home/dyadmin/appdata/git-standard/merge-decisions.jsonl (57/57 shadow decisions escalated as phone noise) vs backtest: 3/3 defective PRs merged on approval alone
