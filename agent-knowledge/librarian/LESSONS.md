...
This is durable, harness-neutral working knowledge for the `librarian` profile.
...
```bash
python3 lessons.py add --key librarian --task "<task type>" \
  --lesson "<reusable lesson>" --evidence "<path, command, or measurement>"
```

and fold them in later with `python3 lessons.py promote --key librarian --apply`,
