# l1-programmer lessons

This is durable, harness-neutral working knowledge for the `l1-programmer` profile.
...
```bash
python3 lessons.py add --key l1-programmer --task "<task type>" \
  --lesson "<reusable lesson>" --evidence "<path, command, or measurement>"
```

and fold them in later with `python3 lessons.py promote --key l1-programmer --apply`,
