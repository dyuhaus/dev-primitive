...
This is durable, harness-neutral working knowledge for the `team-leader` profile.
...
```bash
python3 lessons.py add --key team-leader --task "<task type>" \
  --lesson "<reusable lesson>" --evidence "<path, command, or measurement>"
```

and fold them in later with `python3 lessons.py promote --key team-leader --apply`,
