...
```bash
python3 lessons.py add --key {key} \\
...
```bash
python3 lessons.py add --key {key} --task "<task type>" \\
  --lesson "<reusable lesson>" --evidence "<path, command, or measurement>"
```

and fold them in later with `python3 lessons.py promote --key {key} --apply`,
...
KNOWLEDGE_README = '''<!-- Generated by apply.py knowledge. Do not hand-edit; edit apply.py, then regenerate. -->
