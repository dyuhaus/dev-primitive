...
def assert_generated_paths_are_installable(dry: bool) -> None:
    ...
    Installed Codex skills carry an ABSOLUTE path to `lessons.py`,
...
def install_knowledge(cfg: dict, dry: bool) -> None:
    ...
    # portable text with no machine-specific path in it. `install_harness_skills` is the
...
def refresh_surfaces(cfg: dict, home: Path, dry: bool) -> list:
    ...
    * `set` then refreshed every *present* harness, which installed the Codex,
    ...
    """
    skipped = []
    if surface_installed(home, "codex", cfg):
        assert_generated_paths_are_installable(dry)
...
def main() -> None:
    ...
    if args.action == "set":
        ...
        if args.dry_run:
            ...
            print_table(cfg)
            return
        if not args.no_apply and "codex" in present:
            assert_generated_paths_are_installable(False)
    ...
    if args.action in ("codex", "all"):
        assert_generated_paths_are_installable(args.dry_run)
